...
idx_train <- createDataPartition(dane$SeriousDlqin2yrs, p = 0.80, list = FALSE)
train_data <- dane[idx_train, ]
test_data  <- dane[-idx_train, ]

prop.table(table(train_data$SeriousDlqin2yrs))
prop.table(table(valid_data$SeriousDlqin2yrs))
prop.table(table(test_data$SeriousDlqin2yrs))
```

## Przygotowanie danych pod analize

```{r}
dane_analiza <- as.data.frame(lapply(train_data, function(x) {
  if(is.numeric(x)) {
